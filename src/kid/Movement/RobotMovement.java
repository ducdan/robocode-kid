...
public class RobotMovement {
    ...
    /**
     * Switch Robot Number, <br>
     *  = 1, if r is != to null, <br>
     *  = 2, if ar is != to null, <br>
     *  = 3, if tr is != to null, <br>
     *  = 0, if all are == to null, <br>
    ...
    public void right(double a) {
        switch (srn) {
            case 1:
                r.turnRight(a); Turn = a; Time_Turn = i.getTime();
                break;
            case 2:
                ar.turnRight(a); Turn = a; Time_Turn = i.getTime();
                break;
            case 3:
                tr.turnRight(a); Turn = a; Time_Turn = i.getTime();
        }
    }
    ...
    public void setRight(double a) {
        switch (srn) {
            case 2:
                ar.setTurnRight(a); Turn = a; Time_Turn = i.getTime();
                break;
            case 3:
                tr.setTurnRight(a); Turn = a; Time_Turn = i.getTime();
        }
    }
    ...
    public void ahead(double d) {
        ...
        switch (srn) {
            case 1:
                r.ahead(d); Move = d; Time_Move = i.getTime();
                break;
            case 2:
                ar.ahead(d); Move = d; Time_Move = i.getTime();
                break;
            case 3:
                tr.ahead(d); Move = d; Time_Move = i.getTime();
        }
    }
    ...
    public void setAhead(double d) {
        switch (srn) {
            case 2:
                ar.setAhead(d); Move = d; Time_Move = i.getTime();
                break;
            case 3:
                tr.setAhead(d); Move = d; Time_Move = i.getTime();
        }
    }
    ...
    public void left(double a) {
        switch (srn) {
            case 1:
                r.turnLeft(a); Turn = a; Time_Turn = i.getTime();
                break;
            case 2:
                ar.turnLeft(a); Turn = a; Time_Turn = i.getTime();
                break;
            case 3:
                tr.turnLeft(a); Turn = a; Time_Turn = i.getTime();
        }
    }
    ...
    public void setLeft(double a) {
        switch (srn) {
            case 2:
                ar.setTurnLeft(a); Turn = a; Time_Turn = i.getTime();
                break;
            case 3:
                tr.setTurnLeft(a); Turn = a; Time_Turn = i.getTime();
        }
    }
    ...
    public void back(double d) {
        ...
        switch (srn) {
            case 1:
                r.back(d); Move = d; Time_Move = i.getTime();
                break;
            case 2:
                ar.back(d); Move = d; Time_Move = i.getTime();
                break;
            case 3:
                tr.back(d); Move = d; Time_Move = i.getTime();
        }
    }
    ...
    public void setBack(double d) {
        switch (srn) {
            case 2:
                ar.setBack(d); Move = d; Time_Move = i.getTime();
                break;
            case 3:
                tr.setBack(d); Move = d; Time_Move = i.getTime();
        }
    }
    ...
    public final int setTurnPerpenToAnglewBFwDC(double angle, double dist, double cDist) {
        ...
        if (i.getTime() % 10 == 0) {
            ...
            return i.getRobotMovingSign();
        }
    }

    public final int setTurnPerpenToAnglewBFwDCwFM(double angle, double dist, double cDist) {
        double f = DistFactor(dist, cDist);
        ...
        if (f > 0.85 && f < 1.15) {
            // use flataner
            if (Math.round(Math.random()) == 1 && i.getTeammates() % 10 == 0) {
                setRight(ahead);
                return 1;
            } else {
                setRight(back);
        ...
        } else if (Math.abs(ahead) < Math.abs(back)) {
            setRight(ahead);
            return 1;
        } else {
            setRight(back);
    ...
    public final int turnPerpenToRobotwBFwDCwFM(EnemyData robot, double cDist) {
        return 0;
    }

    /**
     * @todo (4)(setTurnPerpenToRobotwBFwDCwFM) get flatner and this method to work.
     */
    public final int setTurnPerpenToRobotwBFwDCwFM(EnemyData robot, double cDist, Flatner f) {
        EnemyWave w = f.getMaxRiskBulletWave();
        if (w == null)
            return setTurnPerpenToRobotwBFwDC(robot, cDist);
        double dist = i.DistTo(robot);
        double angle = i.AngleTo(robot);
        double riskangle = Utils.getAngle(robot.getX(), robot.getY(), i.getX(), i.getY()) + f.getWorstAngleOffset(robot);
        double deltaangle = angle - riskangle;
        return setTurnPerpenToAnglewBFwDCwFM(Utils.oppositeRelative(angle + deltaangle), dist, cDist);

        /*
                double df = DistFactor(dist, cDist);

                double right = i.RobotBearingTo(angle + (90 * df));
                double left = i.RobotBearingTo(angle - (90 * df));
    ...
    /**
     * @todo (1)(turnToSmoothWalls) make like setTurnToSmoothWalls but with no set
     */
    public final void turnToSmoothWalls() {}

    /**
     * @todo (2)(setTurnToSmoothWalls) adjust velocity so that i never hit a wall
     */
    public final void setTurnToSmoothWalls() {
        double radius = i.getRobotTurnRadius(MyRobotsInfo.MAX_VELOCITY);
        double mX = i.getFutureX(getMove(), getTurn()), mY = i.getFutureY(getMove(), getTurn());
        ...
        dist = ((dist = mX - MyRobotsInfo.MIN_WALL_DIST) < 0.0 ? 0.0 : dist);
        if (dist <= radius) {
        ...
        dist = ((dist = bfw - mX - MyRobotsInfo.MIN_WALL_DIST) < 0.0 ? 0.0 : dist);
        if (dist <= radius) {
            angle = Utils.asin((radius - dist) / radius);
            angle = (dist == 0.0 && radius == 0.0 ? 90.0 : angle);
            RAngle = 90.0 + angle;
        ...
        dist = ((dist = mY - MyRobotsInfo.MIN_WALL_DIST) < 0.0 ? 0.0 : dist);
        if (dist <= radius) {
            angle = Utils.asin((radius - dist) / radius);
            angle = (dist == 0.0 && radius == 0.0 ? 90.0 : angle);
            if (WEST) {
        ...
        dist = ((dist = bfh - mY - MyRobotsInfo.MIN_WALL_DIST) < 0.0 ? 0.0 : dist);
        if (dist <= radius) {
            angle = Utils.asin((radius - dist) / radius);
            angle = (dist == 0.0 && radius == 0.0 ? 90.0 : angle);
            if (WEST) {
        ...
        double frb = i.RobotBearingTo(Utils.relative(RAngle)); // FRONT RIGHT BEARING
        double flb = i.RobotBearingTo(Utils.relative(LAngle)); // FRONT LEFT BEARING
        double brb = i.RobotBearingTo(Utils.oppositeRelative(RAngle)); // BACK RIGHT BEARING
        double blb = i.RobotBearingTo(Utils.oppositeRelative(LAngle)); // BACK LEFT BEARING

        if (i.getRobotMovingSign() > -1) {
            if (isAngleBetween(i.getRobotFrontHeading() + i.getRobotTurn(), LAngle, RAngle) ||
                (!isAngleBetween(i.getRobotFrontHeading() + i.getRobotTurn(), LAngle, RAngle) && twoWalls)) {
                if (Math.abs(frb) <= Math.abs(flb)) {
                    //if (Math.abs(frb) > getRobotTurnRate())
                    //  setVelocity(getVelocity(Math.abs(frb)));
                    setRight(frb);
                } else {
                    //if (Math.abs(flb) > getRobotTurnRate())
                    //  setVelocity(getVelocity(Math.abs(flb)));
                    setRight(flb);
                }
            }
        } else {
            if (isAngleBetween(i.getRobotBackHeading() + i.getRobotTurn(), LAngle, RAngle) ||
                (!isAngleBetween(i.getRobotBackHeading() + i.getRobotTurn(), LAngle, RAngle) && twoWalls)) {
                if (Math.abs(brb) <= Math.abs(blb)) {
                    //if (Math.abs(brb) > i.getRobotTurnRate())
                    //  i.setVelocity(i.getVelocity(Math.abs(brb)));
                    setRight(brb);
                } else {
                    //if (Math.abs(blb) > i.getRobotTurnRate())
                    //  i.setVelocity(i.getVelocity(Math.abs(blb)));
                    setRight(blb);
                }
            }
        }
    ...
    private final double DistFactor(double Dist, double ControllDist) {
        return Math.min(ControllDist / Dist, 2);
    }
